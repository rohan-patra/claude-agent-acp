...
### Tracked-File Watcher Fallback

The built-in `Edit`/`Write` intercept only sees the built-in tools. Changes made through Bash, formatters, MCP tools, subagents, or background processes write straight to disk and bypass Review Changes. This feature adds a **source-agnostic** fallback built on the pinned SDK's filesystem watcher (`FileChanged`/`SessionStart`/`CwdChanged` hook events), coexisting with — and subordinate to — the exact `Edit`/`Write` path.

**Added to `src/tools.ts`** (the coordinator lives inside `createFileEditInterceptor`'s closure, sharing its `isInScope`, cache, and `writeTextFile` routing; `interceptEditWrite` remains the authoritative built-in path). New `FileEditInterceptor` methods:

- `initializeTrackedFileWatch()` — Async init: returns `false` (no watch) when `cwd` is not a Git worktree; otherwise enumerates tracked paths via `git ls-files --cached -z -- .` (`node:child_process`, NUL-delimited) and snapshots each tracked, existing, in-scope text file's **current working-tree contents** (not `HEAD`, not the index) as its reject baseline. Skips binaries (NUL byte / invalid UTF-8), symlinks (`lstat`), unreadable files, `.context/`, and out-of-cwd paths. Sets `watchActive` on success.
- `reinitializeForCwd(newCwd)` — Re-points `resolvedCwd` and re-snapshots (for `CwdChanged`).
- `isWatchActive()` — Whether the watcher is active (gates whether the SessionStart/CwdChanged hooks return `watchPaths`).
- `markExplicitMutation(filePath)` / `clearExplicitMutation(filePath)` — The Edit/Write handshake: PreToolUse marks the path before the built-in tool runs; while marked, the watcher's settle path is a no-op so the native write + interceptor revert + ACP application can't spawn a duplicate proposal. Cleared in `interceptEditWrite`'s `finally` and on `PostToolUseFailure`.
- `onFileChanged(event, filePath, writeTextFile)` — The watcher handler. `unlink` is a hard no-op (never restores/routes, never changes state). Ignores paths outside the startup tracked set. Coalesces rapid events per path (debounce), serializes per path (a `queue` promise chain) while letting different paths proceed independently, then re-reads the settled content and runs a **content-based state machine** whose model matches Zed's verified `write_text_file` behavior: routing a proposal makes Zed persist it to disk immediately (`save_buffer`), a **reject** makes Zed restore the pre-proposal content to disk, and an **accept** writes nothing. `TrackedPathState` holds `committed` (the current reject baseline = last settled/accepted content) and `pending` (`{ proposal, rejectTarget }` | null), plus `explicitMutation`/`debounceGeneration`/`queue`. `pending` also carries a `settled` flag for format-on-save handling. In `processSettledChange`, while a `pending` proposal is outstanding: disk landing on `pending.rejectTarget` = the user rejected (roll `committed` back, clear `pending`); disk landing on `pending.proposal` = the routing write / accept / re-affirm (keep the optimistic `committed = proposal`, set `settled`, leave `pending` so a later reject can still roll back); any other content **while not yet `settled`** = Zed reformatted the proposal on save (`format_on_save`), so **adopt** it as the proposal (`pending.proposal = content`, `committed = content`, `settled = true`, `rejectTarget` unchanged) instead of re-proposing it — this is what stops a formatter from causing a double proposal or, if non-idempotent, an unbounded re-proposal loop; any other content **once `settled`** = a genuine new change (fall through and route it). Otherwise: skip while `explicitMutation` is set (the built-in path owns it), no-op when `content === committed` (also swallows the transient revert echo, since `committed` isn't advanced until after routing), and on a real net change revert disk to `committed`, route through `writeTextFile`, then optimistically set `pending = { proposal, rejectTarget, settled: false }` and advance `committed = proposal` (`syncAfterIntercept` sets the same shape for the built-in Edit/Write path, so format-on-save is handled there too). **Serialization + re-reading live disk at settle time** (not the event's stale content) is what lets the model drop `expectedSelfWrites` entirely: the coordinator's own revert write is only ever observed while `committed` still equals it (→ no-op), and Zed's routing write is observed as `pending.proposal`. ACP failure restores the agent-written content and commits it (mirroring `interceptEditWrite`).

New hook factories in `src/tools.ts`: `createSessionStartWatchHook(getWatchPaths)`, `createCwdChangedWatchHook(onCwdChanged)`, `createFileChangedHook(onFileChanged)`, `createPostToolUseFailureHook({ onEditWriteFailure })`; `createPreToolUseHook` gained an optional `onExplicitMutation` callback (fires for `Write` **and** `Edit`).

**Added to `src/acp-agent.ts`** (all gated on `clientCapabilities.fs.writeTextFile`, composed with any user-provided hooks via the existing spread-then-append pattern):

- `await fileEditInterceptor.initializeTrackedFileWatch()` in `createSession()` **before** `query()` starts.
- `SessionStart` / `CwdChanged` hooks return `hookSpecificOutput.watchPaths: [cwd]` only when `isWatchActive()` (i.e. in a Git worktree); `CwdChanged` re-inits against `new_cwd`.
- A `FileChanged` hook that skips `unlink` and otherwise fire-and-forgets (`void … .catch(log)`) `onFileChanged(event, filePath, …)`, routing through the same `client.writeTextFile({ sessionId, path, content })` bridge as the built-in path. No synthetic `tool_call_update` is emitted for watcher-only changes.
- PreToolUse `onExplicitMutation` → `markExplicitMutation`; a `PostToolUseFailure` hook → `clearExplicitMutation` for `Edit`/`Write` `file_path`, so a failed built-in tool (whose PostToolUse success path never runs) can't permanently suppress later watcher changes.

**Deliberate boundaries** (documented for users in `README.md`): deletions, binaries, symlinks, untracked/post-start files, and tool attribution are all out of scope; rapid writes surface the stable net content; `format_on_save` reformatting is handled (the `settled` adopt); accept/reject is inferred from which content the disk settles on (proposal = accept, reject target = reject), correct except for narrow residual gaps — a new change to the same file in the sub-second window before a reject is observed can mask the reject, a manual edit whose content exactly equals a pending reject target reads as a reject, and a file simultaneously open+edited in Zed's own editor can desync the reject baseline (Zed diffs against the live buffer, not disk). Startup snapshots every tracked text file's content into memory (O(repo text size)); assumes a single active ACP session per working tree (the filesystem protocol can't disambiguate concurrent sessions/adapters on one checkout).

**No new `lib.ts` export** — the feature extends the already-exported `FileEditInterceptor` interface rather than adding a new public type; the coordinator's state-machine helpers stay private to `tools.ts`. Covered by `src/tests/tools.test.ts` (`describe("tracked file watcher")`) and hook-composition/gating cases in `src/tests/create-session-options.test.ts`.
...
The SDK ends the ACP turn (`result` → `end_turn`) when a foreground model invocation finishes, but background work (subagents, Bash, monitors, workflows) keeps running. This fork surfaces that ongoing work in the UI so the user can see what's still executing after the model's foreground reply lands.

> **Turn-settlement half superseded by upstream (v0.59.0 merge).** This feature originally had a **third** sub-feature — "Turn settles at `idle`, not a non-terminal `result`" (a `Turn.settleAtIdle` flag driven by `deferred_tool_use` on a result). Upstream PR **#870** ("hold a turn open while its background subagents are still live") shipped a far more robust reimplementation of turn-settlement-vs-background-work: it holds the turn open on a live-subagent set (`Turn.deferredSettle`, `session.liveBackgroundTasks`, `spawnedTaskIds` ∩ live, `isHeldOpen`/`settleDeferredIfDrained`/`settleOrDefer`) rather than parking at the trailing idle. Per the "prefer upstream's reimplementation" rule (as with Fast mode), the fork's `settleAtIdle`/`deferred_tool_use` sub-feature was **dropped entirely** in that merge. Note upstream **deliberately excludes background shells** from the hold (a dev server outlives every turn), so re-adding a `deferred_tool_use`-keyed hold would directly contradict upstream's design — do **not** reintroduce it. The subagent-attribution map (`subagentParentToolUseIds`, originally fork PR #859) was likewise **subsumed** into upstream's unified `liveBackgroundTasks` registry.

**Two additive features that upstream does NOT provide** (#870 lists background-task-level signals as out of scope):
...
These use a **fork-only** `session.runningTasks` map (distinct from upstream's `liveBackgroundTasks`, which drives the turn-hold + permission attribution). The two coexist: the `task_started`/`task_notification`/`task_updated`/`background_tasks_changed` handlers update **both** — upstream's registry for settlement/attribution, the fork's for plan/card visibility.
...
- `src/acp-agent.ts` — Session fields `runningTasks` (Map) and `runningTaskByToolUseId` (reverse index), alongside (not replacing) upstream's `liveBackgroundTasks`; the `task_started`/`task_notification`/`task_updated`/`background_tasks_changed` handlers update the fork maps in addition to upstream's registry (the merged `background_tasks_changed` case reconciles both); private methods `emitPlan()` and `finalizeRunningTask()`; suppression wiring at the consolidated tool-result emit; plan-combiner integration at `createTaskHook` onChange and TodoWrite/Task* plan-emit sites. Turn-settlement itself (including any `deferred_tool_use` handling) is **upstream's** (`deferredSettle`/`isHeldOpen`) — the fork no longer touches the result/idle settle path here.
...
- **Turn-settlement is upstream's (`deferredSettle`), not the fork's** — The fork's old `settleAtIdle`/`deferred_tool_use` hook was dropped in the v0.59.0 merge in favor of upstream #870. Do **not** reintroduce a fork settle path; if upstream reworks its result/idle settle handlers just let the merge take theirs. The fork only reads `session.runningTasks` for visibility.
- **`background_tasks_changed` handler (merged case)** — Upstream #870 added its own `background_tasks_changed` case (reconciles `liveBackgroundTasks` with REPLACE semantics). The fork also needs this case (reconcile `runningTasks.backgrounded` + `emitPlan`). They are **merged into one `case` block** (upstream's `liveBackgroundTasks` reconcile first, then the fork's `runningTasks` reconcile + `emitPlan`, sharing one `live` Set). If upstream restructures this case, keep both halves in the single case — never let two `case "background_tasks_changed":` labels coexist.
- **task terminal edges update both registries** — In `task_notification` and terminal `task_updated`, upstream deletes from `liveBackgroundTasks`; the fork additionally calls `finalizeRunningTask()`. Keep both calls on both edges.
- **owed-idle accounting is upstream's** — `session.owedTrailingIdles` and the held-turn idle branches (`isHeldOpen`/`settleDeferredIfDrained`) are entirely upstream's #870 code now; the fork no longer adjusts the owed counter. Leave the result/idle owed logic to the merge.
...
- If Zed starts dropping updates outside an active turn: Feature A's late card finalization and Feature B's post-turn plan updates stop rendering. Mitigation: upstream's #870 turn-hold (`deferredSettle`) already keeps the turn open while spawned **subagents** are live, so subagent-driven late updates land in-turn; for non-subagent background work (shells/monitors) that upstream deliberately does not hold for, migrate to an ACP v2 mechanism when it ships. Affected code: the `finalizeRunningTask()` late-emit in `background_tasks_changed` and the final `emitPlan()` call in both terminal task edges.
...
1. **`src/acp-agent.ts`** — Our changes are isolated insertion blocks:
   ...
   - PreToolUse hooks block calling `createPreToolUseHook` with `onPreWrite` (sibling to PostToolUse block); the same block also passes `onExplicitMutation` → `fileEditInterceptor.markExplicitMutation`
   - Tracked-file watcher wiring (all gated on `fs.writeTextFile`): `await fileEditInterceptor.initializeTrackedFileWatch()` in `createSession()` **before** `query()`; a `SessionStart`/`CwdChanged`/`FileChanged`/`PostToolUseFailure` hook block (composed with user hooks via the spread-then-append pattern) — SessionStart/CwdChanged return `watchPaths: [cwd]` only when `isWatchActive()`, FileChanged fire-and-forgets `onFileChanged` skipping `unlink` through the same `client.writeTextFile` bridge, PostToolUseFailure clears the Edit/Write explicit-mutation marker. Keep these gated and never emit a `tool_call_update` for watcher-only changes. If upstream reworks the `hooks` object, keep this block a sibling of the PostToolUse/PreToolUse blocks.
   ...
   - Background-task visibility (plan/card only — turn-settlement is upstream's #870, see the Background-Task Visibility section): `Session` fields `runningTasks`/`runningTaskByToolUseId` init (~2 lines in `createSession()`, alongside upstream's `liveBackgroundTasks`); the `task_started`/`task_notification`/`task_updated`/`background_tasks_changed` handlers update the fork maps **in addition to** upstream's registry (the merged `background_tasks_changed` case reconciles both — never split it into two `case` labels; the terminal edges call both `liveBackgroundTasks.delete` and `finalizeRunningTask`); `emitPlan()` and `finalizeRunningTask()` helper methods; plan-combiner wiring at `createTaskHook` onChange (both blocks use `buildMergedPlanEntries`); suppression at the consolidated tool-result emit (~5 lines, `suppressBackgroundToolResults` wraps `toAcpNotifications`); `runningTasks` forwarded in both `streamEventToAcpNotifications` option types and via `forwardedOptions` (which also carries the fork's `fileEditInterceptor`). **Do not** reintroduce the dropped `Turn.settleAtIdle`/`deferred_tool_use` settle path. Covered by tests in `src/tests/acp-agent.test.ts` (the old "Feature C" describe block was removed).
...
2. **`src/tools.ts`** — Our changes are:
   ...
   - Tracked-file watcher fallback (see the "Tracked-File Watcher Fallback" section): the coordinator lives **inside** `createFileEditInterceptor`'s closure (`TrackedPathState` map — `committed` + `pending` `{ proposal, rejectTarget }`, no self-write set — `trackedFiles` set, `watchActive`, the `snapshotTrackedBaselines`/`processSettledChange`/`syncAfterIntercept`/`writeDisk` helpers, and the module-level `tryReadTextFileContent`/`isGitWorktree`/`listGitTrackedRelativePaths` + `execFileAsync`/`TRACKED_FILE_DEBOUNCE_MS`), plus the new `FileEditInterceptor` methods (`initializeTrackedFileWatch`/`reinitializeForCwd`/`isWatchActive`/`markExplicitMutation`/`clearExplicitMutation`/`onFileChanged`); `interceptEditWrite` gained the marker `finally` + `syncAfterIntercept` handshake but keeps its existing revert/route/cache semantics. New hook factories `createSessionStartWatchHook`/`createCwdChangedWatchHook`/`createFileChangedHook`/`createPostToolUseFailureHook`, and the `onExplicitMutation` option on `createPreToolUseHook` (fires for `Edit` **and** `Write`). Do **not** route watcher changes through a tool-call card; keep `unlink` a hard no-op.
...
Zed <──ACP (ndjson/stdio)──> ClaudeAcpAgent <──Claude Agent SDK──> Claude API
                                    ...
                                    │
                                    └── FileChanged watcher (source-agnostic; tracked-at-start files)
                                        └── net change → revert to baseline → writeTextFile → Review UI
```
