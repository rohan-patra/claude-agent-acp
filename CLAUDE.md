...
- `Session` type: added `fastModeState`, `ultracode`, `workflowsEnabled`, `effortLevel`, `modelInfos` fields
- `buildConfigOptions()`: accepts optional `modelInfos`, `currentEffortLevel`, `fastModeState`, `ultracode` (`{ workflowsEnabled, state }`) params; conditionally pushes `effort_level` and `fast_mode` options. The `ultracode` param adds an `"ultracode"` entry to the effort option (and makes it the effort `currentValue` when on) — it is not a separate option
- `applyConfigOptionValue()`: handles `fast_mode` and `effort_level` config IDs (the if/else chain that `setSessionConfigOption()` and `updateConfigOption()` both call); the `effort` branch special-cases the `"ultracode"` value (toggling the SDK `ultracode` flag); model handler rebuilds config options and resets `ultracode` for non-xhigh models (clearing the SDK flag via the effort-sync)
...
1. **`src/acp-agent.ts`** — Our changes are isolated insertion blocks:
   ...
   - Session config improvements: `buildConfigOptions()` extended with model capability params, `applyConfigOptionValue()` handles `fast_mode`/`effort_level` (the `effort` branch special-cases `"ultracode"`), `prompt()` result handler syncs `fast_mode_state`. **Param-order note:** upstream later added its own `agents`/`currentAgent` params to `buildConfigOptions()` (the Agent dropdown, #794). The merged signature keeps the fork's `fastModeState?`/`ultracode?` params *before* upstream's `agents`/`currentAgent`, so every call site passes `…, fastModeState, ultracode, agents, currentAgent`. If upstream reorders these, realign all call sites (and upstream's positional tests in `acp-agent.test.ts`) together.
