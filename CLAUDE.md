...
### ~~Expanded Model Picker~~ (Removed)

The fork previously maintained a hardcoded `FORK_MODEL_PICKER` list to surface models beyond the SDK's curated entries. This was removed -- the fork now uses the SDK's model list directly (`initializationResult.models`). Users who need specific models can still pin them via the `availableModels` settings allowlist, which is resolved against the SDK's model info via `applyAvailableModelsAllowlist`.
...
1. **`src/acp-agent.ts`** — Our changes are isolated insertion blocks:
   ...
   - ~~Expanded model picker~~ (removed): the fork no longer maintains `FORK_MODEL_PICKER` / `buildForkModelList`. The SDK's model list is used directly when no `availableModels` allowlist is set.
