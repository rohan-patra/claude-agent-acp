...
### Expanded Model Picker (Claude models only)
...
> **Claude models only.** The picker previously also carried non-Anthropic entries (GPT-5.x, Grok, Gemini, Composer, Cursor Auto) routed through a custom model provider/proxy, along with a `family: "custom"` branch that set capability flags inline instead of donating them from an SDK family template. Those entries and the `custom` family were **removed** — the picker is now exclusively the built-in Claude models. If custom models are ever revisited, the shape to restore is a `capabilities?` field on the entry plus a `family === "custom"` bypass in `buildForkModelList` that forces `supportsAutoMode: true`.

**Changes in `src/acp-agent.ts`:**

1. **`FORK_MODEL_PICKER`** — A constant `ReadonlyArray` of the 8 Claude picker entries in native-picker order, each with `value` / `displayName` / `description` / `family` (`"opus" | "sonnet" | "haiku"`). The `value`s are real model IDs/aliases verified to exist in the bundled binary's registry: `fable[1m]` (Fable 5), `opus[1m]` (Opus 5 1M -- rolling alias for latest Opus), `sonnet[1m]` (Sonnet 5 1M), `haiku` (Haiku 4.5), `claude-opus-4-8[1m]` (Opus 4.8 1M), `claude-opus-4-7[1m]` (Opus 4.7 1M), `claude-opus-4-6[1m]` (Opus 4.6 1M), `claude-sonnet-4-6` (Sonnet 4.6). The first entry (`fable[1m]`) is the default selection; it carries `family: "opus"` so it donates the SDK `default` (flagship) capability template. **1M-only policy:** where a model has both a 1M and a non-1M variant, only the 1M entry is listed since 1M is a strict superset; a model with no 1M variant (e.g. `claude-sonnet-4-6`, `haiku`) is listed as-is. **The `[1m]` suffix is required, not cosmetic, for `fable`/`sonnet`:** the bundled CLI's context-window resolver auto-upgrades a bare `fable`/`sonnet` alias to the 1M window only via a native-entitlement auto-detection path that isn't reliably satisfied when the model is set through the SDK's programmatic `query()`/`setModel` control-protocol interface (unlike the interactive CLI). Requesting the `[1m]`-suffixed value explicitly reliably yields the correct 1M window. Verified empirically via a live SDK probe (`getContextUsage()`): bare `"fable"`/`"sonnet"` → `rawMaxTokens: 200000`; `"fable[1m]"`/`"sonnet[1m]"` → `rawMaxTokens: 1000000`/`967000`.
...
**Maintenance:** because the picker is an explicit list, a new Fable/Opus/Sonnet/Haiku generation won't appear automatically — update `FORK_MODEL_PICKER` (and bump the bundled SDK so the new IDs resolve). The default selection is the first entry (currently `fable[1m]`); if Anthropic moves the recommended default, update the list's first entry. Covered by `src/tests/fork-model-list.test.ts`; the resolution/allowlist/auto-mode cases live in `src/tests/acp-agent-settings.test.ts`.
...
- `src/tools.ts` — `RunningTask` type (incl. `workflowName?`); `runningTaskLabel()`, `runningTaskPlanEntries()`, `mergeRunningTaskPlanEntries()`, `buildMergedPlanEntries()`, `suppressBackgroundToolResults()` helpers; `runningTasks?` added to `toAcpNotifications`/`streamEventToAcpNotifications` options.
- `src/acp-agent.ts` — Session fields `runningTasks` (Map) and `runningTaskByToolUseId` (reverse index), alongside (not replacing) upstream's `liveBackgroundTasks`; the `task_started`/`task_notification`/`task_updated`/`background_tasks_changed` handlers update the fork maps in addition to upstream's registry (the merged `background_tasks_changed` case reconciles both); private methods `emitPlan()` and `finalizeRunningTask()` (the latter takes an optional `toolUseIdHint` — `task_notification` passes its own `tool_use_id` so a card whose entry never captured a `toolUseId` can still be finalized); suppression wiring at the consolidated tool-result emit; `cancel()` clears the fork maps + re-emits the plan (see Dependency 4); plan-combiner integration at `createTaskHook` onChange and TodoWrite/Task* plan-emit sites. Turn-settlement itself (including any `deferred_tool_use` handling) is **upstream's** (`deferredSettle`/`isHeldOpen`) — the fork no longer touches the result/idle settle path here.

**Merge-maintenance notes:**

- **Single plan-emit owner** — Every `sessionUpdate: "plan"` routes through the shared combiner `mergeRunningTaskPlanEntries(base, runningTasks)` so the running-task rows are never dropped. `buildMergedPlanEntries(taskState, runningTasks)` delegates to it (used by `emitPlan` + the Task* plan-emit); the TodoWrite plan path calls `mergeRunningTaskPlanEntries(planEntries(input), runningTasks)` directly (its base is the TodoWrite snapshot, not `taskState`, so it can't use `buildMergedPlanEntries`). If you add a new plan-emit, wire its base through `mergeRunningTaskPlanEntries`.
...
- **task terminal edges update both registries** — In `task_notification` and terminal `task_updated`, upstream deletes from `liveBackgroundTasks`; the fork additionally calls `finalizeRunningTask()`. Keep both calls on both edges. `task_notification` passes its `message.tool_use_id` as the `finalizeRunningTask` hint (`task_updated` has none — its patch carries no `tool_use_id`).
...
- Current: User hits Stop during turn N+1. Zed marks all outstanding `in_progress` cards Canceled. `cancel()` now **clears the fork's `runningTasks` + `runningTaskByToolUseId`** (synchronously, before the awaited `interrupt()`) and re-emits the plan without the running rows, so the plan panel and held cards don't leak past the Stop. Because the reverse index is cleared, `suppressBackgroundToolResults` goes inert and a late terminal task edge (`task_notification`/`task_updated`) **no-ops** instead of resurrecting the Canceled card as `completed` — i.e. a Stopped card stays Canceled (a deliberate change from the old "overwrite Canceled with completed" behavior; Stop means stop).
- **Durability**: the clear alone is not enough — the persistent consumer keeps draining, and a buffered `background_tasks_changed` or a raced `task_started` processed during `cancel()`'s own awaits (or after it returns) would otherwise re-add the just-cleared task. So the two fork **add-sites** (the `task_started` running-task upsert and the `background_tasks_changed` running-task reconcile) are gated on **`!session.cancelled`**. `session.cancelled` is set `true` at the top of `cancel()` and reset `false` only in `activateTurn` (the next turn), so the guards suppress re-population for the whole post-Stop window. A genuinely-still-live task legitimately reappears on the next turn's level. Regression-tested by the "cancel() durably clears running tasks" case.
...
1. **`src/acp-agent.ts`** — Our changes are isolated insertion blocks:
   ...
   - Expanded model picker (Claude models only): `FORK_MODEL_PICKER` / `FORK_MODEL_CAPABILITY_FALLBACK` constants + `buildForkModelList()` (defined just before `applyAvailableModelsAllowlist`), and the `createSession()` `allowedModels` branch that uses `buildForkModelList(initializationResult.models)` when no `availableModels` allowlist is set. If upstream changes the `allowedModels`/`getAvailableModels` block, keep our no-allowlist branch pointed at `buildForkModelList`. Non-Anthropic entries and the `family: "custom"` branch were removed — don't reintroduce them.
   - Background-task visibility (plan/card only — turn-settlement is upstream's #870, see the Background-Task Visibility section): `Session` fields `runningTasks`/`runningTaskByToolUseId` init (~2 lines in `createSession()`, alongside upstream's `liveBackgroundTasks`); the `task_started`/`task_notification`/`task_updated`/`background_tasks_changed` handlers update the fork maps **in addition to** upstream's registry (the merged `background_tasks_changed` case reconciles both — never split it into two `case` labels; the terminal edges call both `liveBackgroundTasks.delete` and `finalizeRunningTask`, with `task_notification` forwarding its `tool_use_id` as the `finalizeRunningTask` hint; the two fork **add-sites** — the `task_started` running-task upsert and the `background_tasks_changed` running-task reconcile — are gated on **`!session.cancelled`** so a buffered/raced edge can't resurrect state that `cancel()` cleared, without touching upstream's `liveBackgroundTasks`/`spawnedTaskIds` bookkeeping in the same handlers); `emitPlan()` and `finalizeRunningTask()` helper methods; `cancel()` clears the fork maps (synchronously, before the awaited `interrupt()`) + re-emits the plan (the emit wrapped in try/catch so a broken client transport can't skip `interrupt()`); plan-combiner wiring at `createTaskHook` onChange (both blocks use `buildMergedPlanEntries`); suppression at the consolidated tool-result emit (~5 lines, `suppressBackgroundToolResults` wraps `toAcpNotifications`); `runningTasks` forwarded in both `streamEventToAcpNotifications` option types and via `forwardedOptions` (which also carries the fork's `fileEditInterceptor`). **Do not** reintroduce the dropped `Turn.settleAtIdle`/`deferred_tool_use` settle path. Covered by tests in `src/tests/acp-agent.test.ts` (the old "Feature C" describe block was removed).
...
2. **`src/tools.ts`** — Our changes are:
   ...
   - Background-task helpers appended at end of file: `RunningTask` type (with `workflowName?`), `runningTaskLabel()`, `runningTaskPlanEntries()`, `mergeRunningTaskPlanEntries()`, `buildMergedPlanEntries()`, `suppressBackgroundToolResults()`; `runningTasks?` parameter added to `toAcpNotifications` and `streamEventToAcpNotifications` options types and threaded through.

   If upstream adds new tool handling or modifies plan emission, our additions are all at the end of the file and shouldn't conflict. When wiring new plan emits, route the base entries through `mergeRunningTaskPlanEntries` (the single combiner; `buildMergedPlanEntries` delegates to it).
