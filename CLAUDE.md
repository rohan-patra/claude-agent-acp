...
1. **`FORK_MODEL_PICKER`** — A constant `ReadonlyArray` of the 7 picker entries in native-picker order, each with `value` / `displayName` / `description` / `family`. The `value`s are real model IDs/aliases verified to exist in the bundled binary's registry: `fable[1m]` (Fable 5), `opus[1m]` (Opus 4.8 1M), `claude-opus-4-7[1m]` (Opus 4.7 1M), `claude-opus-4-6[1m]` (Opus 4.6 1M), `sonnet[1m]` (Sonnet 5), `claude-sonnet-4-6` (Sonnet 4.6), `haiku` (Haiku 4.5). The first entry (`fable[1m]`) is the default selection; it carries `family: "opus"` so it donates the SDK `default` (flagship) capability template. (Fable was dropped when Anthropic briefly removed the model and re-added here once it returned to the bundled binary's registry.) **1M-only policy:** where a model has both a 1M and a non-1M variant, only the 1M entry is listed (the non-1M `opus`/`claude-opus-4-7` entries were removed) since 1M is a strict superset; a model with no 1M variant (e.g. `claude-sonnet-4-6`, `haiku`) is listed as-is. **The `[1m]` suffix is required, not cosmetic, for `fable`/`sonnet`:** the bundled CLI's context-window resolver (`qEc`/`eL`/`ep` in the minified binary) auto-upgrades a bare `fable`/`sonnet` alias to the 1M window only via a native-entitlement auto-detection path that isn't reliably satisfied when the model is set through the SDK's programmatic `query()`/`setModel` control-protocol interface (unlike the interactive CLI, which appears to auto-upgrade internally — confirmed via `/status` showing `claude-fable-5[1m]`). Requesting the `[1m]`-suffixed value explicitly (`U_()`'s literal-suffix check) reliably yields the correct 1M window regardless of that auto-detection path, so `fable`/`sonnet` are listed with the suffix just like the Opus entries. Verified empirically via a live SDK probe (`getContextUsage()`): bare `"fable"`/`"sonnet"` → `rawMaxTokens: 200000`; `"fable[1m]"`/`"sonnet[1m]"` → `rawMaxTokens: 1000000`/`967000`.
...
**Maintenance:** because the picker is now an explicit list, a new Fable/Opus/Sonnet/Haiku generation won't appear automatically — update `FORK_MODEL_PICKER` (and bump the bundled SDK so the new IDs resolve). The default selection is the first entry (currently `fable[1m]`); if Anthropic moves the recommended default, update the list's first entry. Covered by `src/tests/fork-model-list.test.ts`; the resolution/allowlist/auto-mode cases live in `src/tests/acp-agent-settings.test.ts`.
