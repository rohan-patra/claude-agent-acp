...
export class ClaudeAcpAgent {
   ...
   *  and drop it from `runningTasks` (+ the reverse index) and the plan. */
  ...
  private async publishTaskPlan(sessionId: string, taskState: TaskState): Promise<void> {
    const runningTasks = this.sessions[sessionId]?.runningTasks ?? new Map<string, RunningTask>();
    await this.client.sessionUpdate({
      sessionId,
      update: {
        sessionUpdate: "plan",
    ...
    const configuredSettings: Options["settings"] =
      typeof userProvidedOptions?.settings === "string"
        ...
        : {
            ...
            ...(userProvidedOptions?.settings ?? modelSettings),
          };
...
const FORK_MODEL_PICKER: ReadonlyArray<{
  ...
  // so the [1m] suffix is required to get the correct context window.
  {
    value: "fable[1m]",
    displayName: "Fable",
    description: "Latest Fable with 1M context",
    family: "opus",
  },
  {
    value: "opus[1m]",
