...
type Session = {
   ...
   *  Fork-only, distinct from `liveBackgroundTasks` above (which drives
   *  turn-hold/permission attribution): this drives the plan-panel + tool-card
   *  visibility that upstream #870 leaves out of scope. */
...
export class ClaudeAcpAgent {
  ...
  async prompt(params: PromptRequest): Promise<PromptResponse> {
    ...
    // main session's transcript stays clean for subsequent turns. Handled
    // outside the turnQueue/consumer model — it owns its own ephemeral Query.
  ...
  private async runConsumer(session: Session, params: { sessionId: string }): Promise<void> {
    ...
    const settleActive = (result: PromptResponse) => {
      ...
      turn.resolve(result);
      this.maybeReleaseIdleWaiters(session);
    };
            ...
            switch (message.subtype) {
              ...
              case "session_state_changed": {
                ...
                if (message.state === "idle") {
                  ...
                  // and must not touch the owed-idle debt — those are only
                  // meaningful at the turn's true turn-over idle, which this is
                  // not.
                }
                break;
              }
              ...
              case "background_tasks_changed": {
                ...
                // membership change, with REPLACE semantics.
                const live = new Set(message.tasks.map((t) => t.task_id));

                // (upstream #870) Reconcile `liveBackgroundTasks` — dropping
                // (or, for subagent entries, unpinning) any entry whose settle
                // bookend (task_notification / terminal task_updated) was lost,
                // so a leaked subagent entry can't defer its spawning turn's
                // settlement forever. Growth of retained (endedPerLevel)
                // subagent entries is bounded by the activation-time sweep in
                // activateTurn, not here. It never ADDS entries (the payload
                // carries no attribution or subagent marker), so the
                // unspecified ordering vs. the edge bookends is safe: a level
                // that precedes its task_started simply no-ops here.
                if (session.liveBackgroundTasks.size > 0) {
                  for (const [taskId, record] of session.liveBackgroundTasks) {
                    if (live.has(taskId)) {
                      ...
                      // after an earlier absent-marking, or that marking was a
                      // racing payload built before the task registered) —
                      // un-end it so a hold waits on it again, and disarm the
                      // activation sweep.
                    ...
                    if (record.isSubagent) {
                      ...
                      // absent — deleting its entry would strand its permission
                      // attribution (#859). Keep the entry but stop any hold
                      // from waiting on the id: an absent id can equally be a
                      // leaked async entry whose settle bookends were lost.
                ...
                // (fork visibility) REPLACE-reconcile the `runningTasks`
                // `backgrounded` flag against the same level rather than
                // pairing edges, so a missed task_started/task_updated bookend
  ...
  /** Release any `/btw` handlers waiting on main-session idle, once no turn is
   *  active and none remain queued. Called from the consumer's turn-settle
   *  paths (settleActive/failActive) so a queued `/btw` proceeds as soon as the
   *  main session goes idle, replacing the old promptRunning finally-block. */
  private maybeReleaseIdleWaiters(session: Session): void {
    if (session.activeTurn || (session.turnQueue?.some((t) => !t.settled) ?? false)) {
      return;
    }
  ...
  private async handleBtwQuestion(
    ...
    // Wait for the main query to idle so output streams don't interleave and
    // so the side query's `resume` reads a stable on-disk transcript. "Busy"
    // under the consumer model means a turn is active or still queued; the
    // consumer drains `idleResolvers` once the queue empties (see settleActive).
    const mainBusy =
      Boolean(session.activeTurn) || (session.turnQueue?.some((t) => !t.settled) ?? false);
    if (mainBusy) {
  ...
  private async createSession(
    ...
    try {
      const content = fsSync.readFileSync(excludePath, "utf-8");
      if (!content.split("\n").some((line) => line.trim() === ".context")) {
        fsSync.appendFileSync(excludePath, `${content.endsWith("\n") ? "" : "\n"}.context\n`);
      }
    } catch {
    ...
    const options: Options = {
      ...
      // model ID overrides) used only when the caller did not supply settings via _meta.
      // Note: when userProvidedOptions.settings is a string (path), we can't merge —
      // the caller has full control and we skip plansDirectory injection.
      settings:
        typeof userProvidedOptions?.settings === "string"
          ? userProvidedOptions.settings
          : {
              plansDirectory: ".context/plans",
              ...(userProvidedOptions?.settings ?? {}),
    ...
    // full Claude model list (Fable 5 + Opus/Sonnet 1M variants) ourselves.
    ...
    // `claude-opus-4-7[1m]`) correctly issues a `setModel` call.
 ...
 *
 * Claude models only: non-Anthropic entries (GPT/Grok/Gemini/Composer, which
 * required a custom model provider or proxy) were removed — this picker is
 * exclusively the built-in Claude models.
 *
...
export function buildForkModelList(sdkModels: ModelInfo[]): ModelInfo[] {
  ...
  return FORK_MODEL_PICKER.map(({ value, displayName, description, family }) => {
    const caps = pickTemplate(family) ?? FORK_MODEL_CAPABILITY_FALLBACK[family];
    return {
      value,
      displayName,
      description,
          ...
          if (registerHooks && !alreadyCached) {
            ...
            registerHookCallback(chunk.id, {
              onPostToolUseHook: async (toolUseId, toolInput, toolResponse) => {
                // Intercept Edit/Write: revert disk write, route through ACP
                // Review UI. Uses the closed-over `toolName` rather than the
                // tool-use cache, which upstream prunes at tool_result time
                // (a PostToolUse hook can fire after that prune — #748).
                if (options?.fileEditInterceptor && (toolName === "Edit" || toolName === "Write")) {
...
export function streamEventToAcpNotifications(
  ...
  const forwardedOptions = {
    ...
    parentToolUseId: message.parent_tool_use_id,
    // Fork additions forwarded verbatim into the internal toAcpNotifications
    // calls below (streamed tool_call/plan emits need the interceptor + the
    // running-task rows).
