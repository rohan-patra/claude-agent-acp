...
export function makeMockQuery(overrides: Record<string, unknown> = {}) {
  return {
    ...
    // Fork addition: createSession seeds the effort level via applyFlagSettings
    // at init when the current model's currentValue is a real effort level, so
    // the base mock must stub it (see the effort_level/ultracode session config).
